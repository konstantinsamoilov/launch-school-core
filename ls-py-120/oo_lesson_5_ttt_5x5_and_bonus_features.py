...
class TTTGame:
    ...
    def _find_critical_square(self, player):
        for row in self.POSSIBLE_WINNING_ROWS:
            if self.board.count_markers_for(player, row) == 4:
                for key in row:
    ...
    def computer_moves(self):
        offensive_move = self._find_critical_square(self.computer)
        defensive_move = self._find_critical_square(self.human)
        second_tier_options = self.board.second_tier_initial_markers()
        third_tier_options = self.board.third_tier_initial_markers()

        if offensive_move:
            choice = offensive_move
        elif defensive_move:
            choice = defensive_move
        elif self.board.squares[13].is_unused():
            choice = 13
    ...
    def five_in_a_row(self, player, row):
    ...
    def is_winner(self, player):
        for row in TTTGame.POSSIBLE_WINNING_ROWS:
            if self.five_in_a_row(player, row):
                return True
        return False
